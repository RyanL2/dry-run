...
from dryrun.sandbox.assemble import Prepared, prepare
...
def _first_blockdev() -> str | None:
    for name in sorted(os.listdir("/dev")):
        path = f"/dev/{name}"
        try:
            if os.stat(path).st_mode & 0o170000 == 0o060000:
                return path
        except OSError:
            continue
    return None
...
def _run(host: _Host, store: Store, cfg: Config, command: str) -> tuple[SpawnResult, RunPaths, Prepared]:
    run = store.new_run()
...
MAIN_IDS = ("I1", "I2", "I3", "I4", "I5", "I6", "I7", "I10a", "I12", "I13", "I14", "I15")
...
def _main_probe(host: _Host, store: Store, cfg: Config) -> list[CanaryResult]:
    ...
    try:
        ...
        try:
            ...
            return [CanaryResult(i, "main probe", False, f"probe did not report: {err}") for i in MAIN_IDS]
