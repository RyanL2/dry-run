...
| **H2** | Changes outside the workspace | the command attempted a write outside the workspace, `/tmp` and home cache dirs. Such writes fail with EROFS in the shadow, which sets flag `ro_write_blocked`. `/tmp` and cache changes are exempt | **ask, hard**, mode `rerun`. Writes outside the workspace and `/tmp` are never committed, so the only way they happen is a real run you approve |
...
| **H5** | Credential access | a per-run canary token from a decoy credential file appears in stdout/stderr or in ChangeSet content (plain, base64 or hex). Without network, that is exactly how a credential read would reach the agent or the real disk | **deny, hard**, with the reason "exposed credential decoy <path>" |
| **H6** | Resource abuse | wall clock exceeded; `TasksMax` or `MemoryMax` hit; disk budget or free-space floor hit; syscall trace over 16 MiB (cut short, so execs and network attempts may be missing) | **ask, hard**, flagged `timeout` or `resource_limit` |
